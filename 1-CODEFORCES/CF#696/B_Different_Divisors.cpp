...
void solve()
{
    ...
    ll ans = 1;
    ll nxt, b1, b2;
    if (binary_search(all(prm), diff + 1))
    {
        ans *= (diff + 1);
        b1 = (diff + 1);
    }
    ...
    {
        ll n1_id = upper_bound(all(prm), diff + 1) - prm.begin();
        ans *= prm[n1_id];
        b1 = prm[n1_id];
    }

    if (binary_search(all(prm), b1 + diff))
    {
        ans *= (b1 + diff);
        b2 = b1 + diff;
    }
    ...
    {
        ll n2_id = upper_bound(all(prm), b1 + diff) - prm.begin();
        ans *= prm[n2_id];
        b2 = prm[n2_id];
    }
