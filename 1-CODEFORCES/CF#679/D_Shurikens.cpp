...
int main()
{
    ...
    //Min-Heap Implementation of Min-Heap
    ...
    for (int i = 2 * n - 1; i >= 0; i--)
    {
        if (s[i] == '+')
        {
            if (pq.empty())
            {
                // you have removed more than added
                cout << "NO\n";
                return 0;
            }
            else
            {
                // The element added was pq.top()
                ans.pb(pq.top());
                pq.pop();
            }
        }
        else
        {
            ...
            else
            {
                if (a[end] > pq.top())
                {
                    // You have to remove the cheapest one
                    cout << "NO\n";
                    return 0;
                }
                else
